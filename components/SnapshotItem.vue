...
<template>
    <div class="flex bg-secondary rounded-xl py-4">
        ...
        <p class="text-center w-[16%]">{{ Math.round(snapshot.image_size * 100) / 100 }} / {{ snapshot.disk_size }} GB</p>
        <p class="text-center w-[16%]">{{ Math.round((snapshot.image_size * 0.011) * 1000) / 1000 }}€/mes</p>
        <p class="text-center w-[20%]">{{ dateToTimeAgo(timestamp) }}</p>
        <div class="flex justify-center text-center w-[10%]">
            <button @click="deploySnapshot()" class="bg-[#202020] px-2 rounded-lg hover:scale-110">Cargar</button>
        </div>
    </div>
</template>